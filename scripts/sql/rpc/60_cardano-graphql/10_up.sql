...
CREATE OR REPLACE VIEW cardano_graphql."DelegationVote" AS
SELECT
...
CREATE OR REPLACE VIEW cardano_graphql."DrepRegistration" AS
SELECT
...
SELECT
  ...
  quantity,
  tx_id,
  ma_tx_mint.ident AS "ident"  -- exposed the FK to multi_asset.id for cheaper relations to Asset view
FROM ma_tx_mint
JOIN multi_asset
...
SELECT
  ...
  quantity,
  tx_out_id,
  ma_tx_out.ident AS "ident"  -- exposed the FK to multi_asset.id for cheaper relations to Asset view
FROM ma_tx_out
JOIN multi_asset
...
CREATE OR REPLACE VIEW cardano_graphql."Utxo" AS SELECT
  ...
  tx_out.inline_datum_id AS "inline_datum_id",
  tx_out.reference_script_id AS "reference_script_id"
FROM cardano_db_sync.tx
JOIN cardano_db_sync.tx_out
  ON tx.id = tx_out.tx_id
LEFT OUTER JOIN cardano_db_sync.tx_in
