...
echo 'Please input your node treasury cardano address (must be for the same network!)'
NODE_TREASURY_ADDRESS_INPUT=$(gum input --prompt "Ticker: " --placeholder "${NODE_TREASURY_ADDRESS}" --prompt.foreground 99 --cursor.foreground 99 --width 50)
if [[ -n "$NODE_TREASURY_ADDRESS_INPUT" ]]; then
  update_env_var ".env" "NODE_TREASURY_ADDRESS" ${NODE_TREASURY_ADDRESS_INPUT}
  NODE_TREASURY_ADDRESS=${NODE_TREASURY_ADDRESS_INPUT}
fi
...
if gum confirm "Download Cardano DB Sync bootstrap snapshot? (non-official, community maintained sources)" --default=true --affirmative "Download" --negative "Skip"; then
    download_csnapshot
    docker compose up -d
fi
