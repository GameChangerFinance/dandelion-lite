...
echo "ℹ️ Making a backup of project '$projectName'..."
echo

volumeNames=$(docker volume ls -q)

echo "ℹ️ About to backup all these volumes into these backup files (${backupDir}<volume_name_without_project_name.tar.gz>):"
...
echo
echo "⚠️ Warning: all containers will be turned off to proceed with a clean state !"
...
echo "ℹ️ Setting a dummy database password '$backupPassword' for safe backup sharing..."
...
for volumeName in $volumeNames; do
    if [[ $volumeName == "$projectName"* ]]; then   # True if $volumeName starts with a projectName.
	fileName=$(echo $volumeName | sed "s/^${projectName}//")
		echo "ℹ️ Creating '${fileName}.tar.gz'..."
